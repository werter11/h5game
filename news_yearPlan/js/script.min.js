...
var winWidth = window.innerWidth, winHeight = window.innerHeight, baseUrl = "/news_yearPlan/", funObj, Game = {
    init: function () {
        var a = this;
    ...
    preload: function () {
        ...
        a.left = .68 * ratio,
            ...
            a.num = 0,
            a.walking = !1,
            a.game.load.spritesheet("dice", baseUrl + "images/dice_group.png", 50, 50, 10),
            a.game.load.spritesheet("player", baseUrl + "images/player.png", 82, 108, 8),
            a.game.load.image("background", baseUrl + "images/background.png"),
            a.game.load.spritesheet("speak_group", baseUrl + "images/speak_group.png", 338, 232, 4),
            a.game.load.spritesheet("diceBtn", baseUrl + "images/btn_group.png", 180, 124, 2),
    ...
    create: function () {
        ...
        funObj = this,
            ...
            a.goDes = function () {
                ...
                var b = jsonData[a.index];
                if (a.direction ? a.player.play(b.dir) : a.player.play(b._dir),
                    a.num > 0) {
                    a.direction = !0,
                        a.walking = !0,
                        a.index++,
                        a.collisionPlayer.x = jsonData[a.index].x / 100 * ratio + a.left,
                        a.collisionPlayer.y = jsonData[a.index].y / 100 * ratio + a.top;
                    var c = a.index - 1;
                    Math.abs(parseInt(jsonData[a.index].x) - parseInt(jsonData[c].x)) > 0 ? (parseInt(jsonData[a.index].x) - parseInt(jsonData[c].x) > 0 ? a.player.body.velocity.x = a.player_v : a.player.body.velocity.x = -a.player_v,
                        a.player.body.setSize(1, a.player.height, -.1 * a.player.width, 0)) : (a.player.body.setSize(a.player.width, 1, 0, .1 * a.player.height),
            ...
            a.initSpeak = function (b, c) {
                if (c.result && result[c.result].num++,
                    b) {
                    switch (a.speak_dir = b,
                    a.text.text = c.text,
                    $(".infoCon").fadeIn(200),
                    a.diceCan = !1,
                    ...
                    "stop" == c.alertInfo.status && (a.diceCan = !1),
                        $(document).one("tap", ".page3 .infoCon", function () {
                            if ("stop" == c.alertInfo.status)
                                ...
                                b > 0 ? (result.r6.num++,
                                    a.direction = !0) : (result.r5.num++,
                                        a.direction = !1),
                                    a.num = b,
                                    a.goDes(),
                                    a.diceCan = !1
                            }
                            a.speak_con.visible = !0,
                                $(this).fadeOut(200)
                        })
                } else
                    a.diceCan = !0
                ...
                var b = 3
                    , c = setInterval(function () {
                        return b--,
                            b <= 0 ? (a.diceCan = !0,
                                clearInterval(c),
                                void (a.countDown.visible = !1)) : void (a.numText.text = b)
                    }, 1e3)
            }
            ,
            a.playerCollision = function () {
                a.flag = !0,
                    ...
                    a.walking && (a.direction ? a.num -= 1 : a.num += 1,
                        a.goDes())
            }
            ,
            a.initPlayer = function () {
                ...
                b.onComplete.add(function () {
                    a.player.animations.play("player_right"),
                        c.start()
                }, this),
            ...
            a.dice_btn = a.game.add.sprite(2.3 * ratio, winHeight - .2 * ratio, "diceBtn", 0),
            a.dice_btn.width = 1.8 * ratio,
            a.dice_btn.height = 1.24 * ratio,
            a.dice_btn.anchor.set(0, 1),
            a.game.physics.arcade.enable(a.dice_btn),
            a.dice_btn.body.setSize(2 * a.dice_btn.width, 5, 0, -.3 * ratio),
            ...
            a.dice = a.game.add.sprite(2.1 * ratio, winHeight - 1 * ratio, "dice", 4),
            ...
            a.collisionPlayer.anchor.set(.5, .5),
            ...
            a.numCon.width = .65 * ratio,
            a.numCon.height = .65 * ratio,
    ...
    update: function () {
        var a = this;
        switch (a.game.physics.arcade.collide(a.player, a.collisionPlayer, a.playerCollision, null, this),
        ...
        a.countDown.x = a.player.x - .45 * a.player.width,
        a.countDown.y = a.player.y - 1.4 * a.player.height,
        a.text.y = a.speak.y + .5 * a.speak.height,
        a.text.x = a.speak.x + .55 * a.speak.width,
        a.speak_dir) {
            case "0":
                a.speak_con.x = a.player.x - .9 * a.speak.width,
                    a.speak_con.y = a.player.y - 1.2 * a.speak.height;
                break;
            case "1":
                a.speak_con.x = a.player.x - .1 * a.speak.width,
                    a.speak_con.y = a.player.y - 1.2 * a.speak.height;
                break;
            case "2":
                a.speak_con.x = a.player.x - .8 * a.speak.width,
                    a.speak_con.y = a.player.y + .1 * a.speak.height
        }
        if (a.cursors.left.isDown && a.flag && (a.direction = !1,
            a.num = -1,
            a.goDes(),
            a.flag = !1),
            a.cursors.right.isDown && a.flag && (a.direction = !0,
                a.num = 1,
                a.goDes(),
                a.flag = !1),
            58 == a.index)
    ...
    load: function (a, b) {
        ...
        c.setAttribute("type", "text/javascript"),
            ...
            /msie/.test(window.navigator.userAgent.toLowerCase()) ? c.onreadystatechange = function () {
